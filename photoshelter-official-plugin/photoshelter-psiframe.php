...
class PSIframe {
	...
	function insertImg($I_ID, $G_ID, $width, $f_html, $f_bar = false, $g_name, $f_cap = false) {
		...
		if ($f_html == 'true') {
			//$insert_string = '<a href=\''.$photoshelter_link.'\'><img src=\'' . self::BASE_URL . '/img-get/' . $I_ID .'/s/'. $width.'/' . $height . '/' .$file_name . '\' border=\'0\' title=\''.$img_title.'\' alt=\'' .$alt . '\' width=\'' . $width . '\'></a>';
			$insert_string = '<a href=\''.$photoshelter_link.'\'><img src=\'' . self::BASE_URL . '/img-get/' . $I_ID .'/s/'. $width.'/' . $height . '/' .$file_name . '\' border=\'0\' title=\''.$img_title.'\' alt=\'' .$alt . '\'></a>';
		} else {
			$height += 20;
			$insert_string = "[photoshelter-img width='".$width."' height='".$height."' i_id='".$I_ID."' buy='".$b."']";
			//$insert_string = "[photoshelter-img i_id='".$I_ID."' buy='".$b."']";
		}
		
		if($f_cap == 'on') {
			$insert_string = '[caption align=\"alignnone\" width=\"'.$width.'\" caption=\"'.$alt.'\"]'.$insert_string.'[/caption]';
		}
